Shader "Unlit/HighlightMask"
{
       Properties
    {
    	...
    	// 遮罩系数
    	_MaskFactor("MaskFactor", Range(-10,20)) = 1
    	// 漫反射颜色
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			struct v2f
			{
				...
				fixed3 lightTangentWorldDir : TEXCOORD2;
				fixed3 viewTangentWorldDir : TEXCOORD3;
			...
			sampler2D _MaskTex; // 遮罩纹理_MaskTex
			float4 _MaskTex_ST; // 遮罩纹理的平移和缩放
			fixed4 _MaskFactor; // 遮罩系数
			...
			v2f vert (appdata_full v)
			{
				...
				o.lightTangentWorldDir = normalize(mul(Matrix_MtoT, lightModelSpaceDir));
				o.viewTangentWorldDir = normalize(mul(Matrix_MtoT, viewModelSpaceDir));
				return o;
			}
			...
			fixed3 getSpecularLightColor(in fixed3 semi_light_view, in fixed3 normalTangentDir, in float MaskValue)
			{
				return _LightColor0.rgb * _SpecularColor.rgb * pow(max(0, dot(semi_light_view, normalTangentDir)), _SpecularFactor) * MaskValue;
			}
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed3 semi_light_view = normalize(i.lightTangentWorldDir + i.viewTangentWorldDir);
				//calculate mask value
				fixed3 maskValue = tex2D(_MaskTex, i.uvMask).r * _MaskFactor;
				fixed3 color = getAmbientColor(blendingColor)	// 环境光颜色
					+ getLambertDiffuseLightColor(normal, i.lightTangentWorldDir, blendingColor)		//漫反射光颜色
					+ getSpecularLightColor(semi_light_view, normal, maskValue);		//高光反射光颜色
